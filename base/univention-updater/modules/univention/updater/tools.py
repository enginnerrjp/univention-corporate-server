...
class UniventionUpdater(object):
    ...
    def _get_releases(self):
        ...
        try:
            if not self.repourl.path:
                try:
                    _code, _size, data = self.server.access(None, '/univention-repository/releases.json')
            ...
            try:
                _code, _size, data = self.server.access(None, 'releases.json')
    ...
    def get_sh_files(repositories, verify=False):
        # type: (Iterable[Tuple[UCSHttpServer, _UCSRepo]], bool) -> Iterator[Tuple[_UCSServer, _UCSRepo, Optional[str], str, str]]
        """
